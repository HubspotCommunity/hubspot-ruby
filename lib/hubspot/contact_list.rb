  ...
  class ContactList
    ...
    class << self
      ...
      def create!(opts={})
        ...
        portal_id = opts.delete(:portal_id) { HubspotLegacy::Config.portal_id }

        response = HubspotLegacy::Connection.post_json(LISTS_PATH, params: {}, body: opts.merge({ dynamic: dynamic, portal_id: portal_id}) )
        new(response)
      end
      ...
      def find(ids)
        ...
        else raise HubspotLegacy::InvalidParams, 'expecting Integer or Array of Integers parameter'
        end
    ...
    def update!(opts={})
      response = HubspotLegacy::Connection.post_json(LIST_PATH, params: { list_id: @id }, body: opts)
    ...
    def destroy!
      response = HubspotLegacy::Connection.delete_json(LIST_PATH, { list_id: @id })
    ...
    def contacts(opts={})
      ...
      if bypass_cache || @contacts.nil?
        ...
        response = HubspotLegacy::Connection.get_json(path, HubspotLegacy::ContactProperties.add_default_parameters(opts))
        @contacts = response['contacts'].map! { |c| HubspotLegacy::Contact.from_result(c) }
    ...
    def refresh
      response = HubspotLegacy::Connection.post_json(REFRESH_PATH, params: { list_id: @id, no_parse: true }, body: {})
      response.code == 204
    end
    ...
    def add(contacts)
      ...
      response = HubspotLegacy::Connection.post_json(ADD_CONTACT_PATH, params: { list_id: @id }, body: { vids: contact_ids })
    ...
    def remove(contacts)
      contact_ids = [contacts].flatten.uniq.compact.map(&:id)
      response = HubspotLegacy::Connection.post_json(REMOVE_CONTACT_PATH, params: { list_id: @id }, body: { vids: contact_ids })
      response['updated'].sort == contact_ids.sort
    end
