module Hubspot
  class Properties
    ...
    class << self
      ...
      def all(connection, path, opts={}, filter={})
        response = connection.get_json(path, opts)
        filter_results(response, :groupName, filter[:include], filter[:exclude])
      end

      def groups(connection, path, opts={}, filter={})
        response = connection.get_json(path, opts)
        filter_results(response, :name, filter[:include], filter[:exclude])
      end

      def create!(connection, path, params={})
        post_data = valid_property_params(params)
        return nil if post_data.blank?
        connection.post_json(path, params: {}, body: post_data)
      end

      def update!(connection, path, property_name, params={})
        post_data = valid_property_params(params)
        return nil if post_data.blank?
        connection.put_json(path, params: { property_name: property_name }, body: post_data)
      end

      def delete!(connection, path, property_name)
        response = connection.delete_json(path, property_name: property_name)
        response.parsed_response
      end

      def create_group!(connection, path, params={})
        post_data = valid_group_params(params)
        return nil if post_data.blank?
        connection.post_json(path, params: {}, body: post_data)
      end

      def update_group!(connection, path, group_name, params={})
        post_data = valid_group_params(params)
        return nil if post_data.blank?
        connection.put_json(path, params: { group_name: group_name }, body: post_data)
      end

      def delete_group!(connection, path, group_name)
        response = connection.delete_json(path, group_name: group_name)
        response.parsed_response
      end
