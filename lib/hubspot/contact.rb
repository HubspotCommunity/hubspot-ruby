class Hubspot::Contact < Hubspot::Resource
  ...
  class << self
    ...
    def find_by_email(email, show_list_memberships=nil)
      options = {}
      options[:showListMemberships] = show_list_memberships unless show_list_memberships.nil?
      response = Hubspot::Connection.get_json(FIND_BY_EMAIL_PATH, options.merge({ email: email} ))
      from_result(response)
    end

    def find_by_user_token(token, show_list_memberships=nil)
      options = {}
      options[:showListMemberships] = show_list_memberships unless show_list_memberships.nil?
      response = Hubspot::Connection.get_json(FIND_BY_USER_TOKEN_PATH, options.merge({ token: token}))
      from_result(response)
    end
