module Hubspot
  class Connection
    ...
    class << self
      ...
      def put_json_with_token(path, token, opts)
        url = generate_url_using_oauth(path, opts[:params])
        response = put(url, body: opts[:body].to_json, headers: { 'Content-Type' => 'application/json', 'Authorization' => "Bearer #{token}" }, format: :json)
        log_request_and_response url, response, opts[:body]
        raise(Hubspot::RequestError.new(response)) unless response.success?
        response.parsed_response
      end
      ...
      def generate_url_using_oauth(path, params={}, options={})
        Hubspot::Config.ensure! :hapikey
        path = path.clone
        params = params.clone
        base_url = options[:base_url] || Hubspot::Config.base_url
