    ...
    context "when the contact already exists" do
      it "updates the contact" do
        VCR.use_cassette("contacts/update_contact", record: :none) do
          ...
          contact = Hubspot::Contact.createOrUpdate(
            existing_contact.email,
            params
          )

          assert_requested :post, hubspot_api_url("/contacts/v1/contact/createOrUpdate/email/#{existing_contact.email}?hapikey=demo")
