describe HubspotLegacy::ContactList do
  ...
  let(:static_list) { HubspotLegacy::ContactList.all(static: true, count: 3).last }
  let(:dynamic_list) { HubspotLegacy::ContactList.all(dynamic: true, count: 1).first }
  ...
  describe '#initialize' do
    subject { HubspotLegacy::ContactList.new(example_contact_list_hash) }

    it { should be_an_instance_of HubspotLegacy::ContactList }
  ...
  describe '#contacts' do
    ...
    let(:list) { HubspotLegacy::ContactList.new(example_contact_list_hash) }
    ...
    expect_count_and_offset do |params|
      HubspotLegacy::ContactList.find(1).contacts(params)
    end
  end

  describe '.create' do
    subject{ HubspotLegacy::ContactList.create!({ name: name }) }
    ...
    context 'with all required parameters' do
      ...
      context 'adding filters parameters' do
        ...
        it 'returns a ContactList object with filters set' do
          ...
          list_with_filters = HubspotLegacy::ContactList.create!({ name: name, filters: filters_param })
          expect(list_with_filters).to be_a(HubspotLegacy::ContactList)
    ...
    context 'without all required parameters' do
      ...
      it 'raises an error' do
        expect { HubspotLegacy::ContactList.create!({ name: nil }) }.to raise_error(HubspotLegacy::RequestError)
      end
    end
  end
  ...
  describe '.all' do
    context 'all list types' do
      ...
      it 'returns by default 20 contact lists' do
        lists = HubspotLegacy::ContactList.all
        expect(lists.count).to eql 20

        list = lists.first
        expect(list).to be_a(HubspotLegacy::ContactList)
        expect(list.id).to be_an(Integer)
      end

      expect_count_and_offset { |params| HubspotLegacy::ContactList.all(params) }
    end
    ...
    context 'static lists' do
      ...
      it 'returns by defaut all the static contact lists' do
        lists = HubspotLegacy::ContactList.all(static: true)
    ...
    context 'dynamic lists' do
      ...
      it 'returns by defaut all the static contact lists' do
        lists = HubspotLegacy::ContactList.all(dynamic: true)
        expect(lists.count).to be > 20
  ...
  describe '.find' do
    context 'given an id' do
      ...
      subject { HubspotLegacy::ContactList.find(id) }
      ...
      context 'Wrong parameter type given' do
        it 'raises an error' do
          expect { HubspotLegacy::ContactList.find(static_list) }.to raise_error(HubspotLegacy::InvalidParams)
        end
      end

      context 'when the contact list is not found' do
        it 'raises an error' do
          expect { HubspotLegacy::ContactList.find(-1) }.to raise_error(HubspotLegacy::RequestError)
        end
      end
    end
    ...
    context 'given a list of ids' do
      ...
      it 'find lists of contacts' do
        lists = HubspotLegacy::ContactList.find([2,3,4])
        list = lists.first
        expect(list).to be_a(HubspotLegacy::ContactList)
    ...
    context "for a static list" do
      it "adds the contact to the contact list" do
        VCR.use_cassette("contact_lists/add_contact") do
          contact = HubspotLegacy::Contact.create("email@example.com")
          contact_list_params = { name: "my-contacts-list" }
          contact_list = HubspotLegacy::ContactList.create!(contact_list_params)
  ...
  describe '#remove' do
    ...
    context 'static list' do
      ...
      it 'returns false if the contact cannot be removed' do
        contact_not_present_in_list = HubspotLegacy::Contact.new(example_contact_hash)
    ...
    context 'dynamic list' do
      it 'raises error if try to remove a contact from a dynamic list' do
        ...
        expect { dynamic_list.remove(contact) }.to raise_error(HubspotLegacy::RequestError)
      end
    end
  end
  ...
  describe '#update!' do
    ...
    let(:contact_list) { HubspotLegacy::ContactList.new(example_contact_list_hash) }
  ...
  describe '#destroy!' do
    ...
    let(:contact_list) { HubspotLegacy::ContactList.create!({ name: "newcontactlist_#{Time.now.to_i}"}) }
